...
{
	size_t logicalSize = GetSize(pool);
...
stdPool* stdPool_Create(size_t type, unsigned int size, ...)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		void* tmpDataMemory = calloc(1, data->elementSize);
		assert(tmpDataMemory != NULL);
		memcpy_s(tmpDataMemory, data->elementSize, vaNext, data->elementSize);
		Data tmpData = {
			.data = tmpDataMemory,
			.isFree = false
		};
